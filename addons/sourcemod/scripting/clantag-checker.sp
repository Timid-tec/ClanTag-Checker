/*  [CS:GO] Clantag-Checker: Check clantags & change them to anything else.
 *
...
{
	name = "Clantag-Checker", 
	author = PLUGIN_AUTHOR, 
	description = "Check clantags & change them to anything else", 
	version = "4.2.2", 
...
public void OnPluginStart()
{
	...
	//ConVar List
	g_cvTagName = CreateConVar("sm_check_tag", " ", "Tag to check for switching (def. Test )");
...
public Action checkTag(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (StrContains(sTag, nametag, true) != -1)
	{
		...
		PrintToChat(client, "Switching %s to Test", nametag);
	}
